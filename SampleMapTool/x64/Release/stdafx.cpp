...
ID3D12Resource* CreateTextureUploadBuffer(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, void* pTextureData, UINT nWidth, UINT nHeight, DXGI_FORMAT format, ID3D12Resource** ppd3dUploadBuffer)
{
	...
	// Calculate the upload buffer size.

	const UINT64 uploadBufferSize = nWidth * nHeight * 4; // Assuming 4 bytes per pixel
	...
	if (pTextureData)
	{
		UINT8* pTextureDataBegin = NULL;
		D3D12_RANGE range = { 0, 0 }; // No CPU read
		(*ppd3dUploadBuffer)->Map(0, &range, reinterpret_cast<void**>(&pTextureDataBegin));
		memcpy(pTextureDataBegin, pTextureData, uploadBufferSize);
		(*ppd3dUploadBuffer)->Unmap(0, nullptr);

		pd3dCommandList->CopyResource(pd3dTexture, *ppd3dUploadBuffer);
	}
